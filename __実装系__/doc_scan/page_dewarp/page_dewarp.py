...
def get_contours(name, small, pagemask, masktype):
    ...
    # Python 3: findContours now returns only 2 values
...
def optimize_params(name, small, dstpoints, span_counts, params):
    ...
    print('  optimizing {} parameters...'.format(len(params)))
    start = datetime.datetime.now()
    res = scipy.optimize.minimize(objective, params,
                                method='Powell')
    end = datetime.datetime.now()
    print('  optimization took {} sec.'.format(round((end-start).total_seconds(), 2)))
    print('  final objective is {}'.format(res.fun))
    params = res.x
...
def get_page_dims(corners, rough_dims, params):
    ...
    print('  got page dims {} x {}'.format(dims[0], dims[1]))  # print文を修正
...
def remap_image(name, img, small, page_dims, params):
    ...
    height_small = int(height / REMAP_DECIMATE)  # 整数除算に修正
    width_small = int(width / REMAP_DECIMATE)    # 整数除算に修正
    ...
    map_x = image_x_coords.astype(np.float32)
    map_y = image_y_coords.astype(np.float32)
...
def main():
    ...
    if len(sys.argv) < 2:
        print('usage: {} IMAGE1 [IMAGE2 ...]'.format(sys.argv[0]))
        sys.exit(0)
